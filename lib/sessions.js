...
function createSessionManager(opts) {
  ...
  var currentModel = null;      // global fallback model (set by SDK bridge)
  var availableModels = [];     // available models list (set by SDK bridge)
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      // Extract the last model from history (scan backwards for model_info)
      var lastModel = null;
      for (var h = loaded[i].history.length - 1; h >= 0; h--) {
        if (loaded[i].history[h].type === "model_info" && loaded[i].history[h].model) {
          lastModel = loaded[i].history[h].model;
          break;
        }
      }
      var session = {
        ...
        model: lastModel,             // Track per-session model (extracted from history)
        blocks: {},
        sentToolResults: {},
        pendingPermissions: {},
        pendingAskUser: {},
  ...
  function createSession() {
    ...
    var session = {
      ...
      messageQueue: null,
      cliSessionId: null,
      model: null,                // Track per-session model
      blocks: {},
      sentToolResults: {},
      pendingPermissions: {},
      pendingAskUser: {},
  ...
  function switchSession(localId) {
    ...
    // Send the session's current model (or fallback to global/default)
    var sessionModel = session.model || currentModel || "default";
    send({ type: "model_info", model: sessionModel, models: availableModels });
  ...
  function resumeSession(cliSessionId, opts) {
    ...
    // Extract the last model from history (scan backwards for model_info)
    var lastModel = null;
    for (var i = cliHistory.length - 1; i >= 0; i--) {
      if (cliHistory[i].type === "model_info" && cliHistory[i].model) {
        lastModel = cliHistory[i].model;
        break;
      }
    }
    ...
    var session = {
      ...
      model: lastModel,           // Track per-session model
      blocks: {},
      sentToolResults: {},
      pendingPermissions: {},
      pendingAskUser: {},
