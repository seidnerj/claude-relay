...
var COLOR_MODE_KEY = "relay-color-mode";
var colorMode = "auto"; // "auto" | "light" | "dark"
var darkMq = window.matchMedia("(prefers-color-scheme: dark)");
var userThemeId = "claude"; // the theme the user explicitly chose (before auto-pairing)
...
export function setColorMode(mode) {
  ...
  applyAutoMode();
  if (pickerEl) updateModeToggle();
}

/**
 * Find a paired light/dark variant of a theme.
 * Convention: "solarized" ↔ "solarized-light", "gruvbox" ↔ "gruvbox-light"
 */
function findPairedTheme(id, wantLight) {
  if (wantLight && themes[id + "-light"]) return id + "-light";
  if (!wantLight && themes[id.replace(/-light$/, "")] && id.endsWith("-light")) return id.replace(/-light$/, "");
  return id;
}

function applyAutoMode() {
  if (colorMode === "auto") {
    var wantLight = !darkMq.matches;
    var paired = findPairedTheme(userThemeId, wantLight);
    if (paired !== currentThemeId) applyTheme(paired);
  } else if (colorMode === "light") {
    var paired = findPairedTheme(userThemeId, true);
    if (paired !== currentThemeId) applyTheme(paired);
  } else if (colorMode === "dark") {
    var paired = findPairedTheme(userThemeId, false);
    if (paired !== currentThemeId) applyTheme(paired);
  }
}
...
function createThemeItem(id, theme) {
  ...
  item.addEventListener("click", function (e) {
    e.stopPropagation();
    userThemeId = id;
    try { localStorage.setItem(STORAGE_KEY + "-user", id); } catch (ex) {}
...
function updateModeToggle() {
  if (!pickerEl) return;
  var btns = pickerEl.querySelectorAll(".color-mode-btn");
  for (var i = 0; i < btns.length; i++) {
    btns[i].classList.toggle("active", btns[i].dataset.mode === colorMode);
  }
}
...
function buildPickerContent() {
  ...
  var modeLabels = { auto: "Auto", light: "Light", dark: "Dark" };
  for (var mi = 0; mi < modes.length; mi++) {
    var modeBtn = document.createElement("button");
    modeBtn.className = "color-mode-btn";
    if (modes[mi] === colorMode) modeBtn.classList.add("active");
    modeBtn.dataset.mode = modes[mi];
    modeBtn.textContent = modeLabels[modes[mi]];
...
export function initTheme() {
  ...
  // Restore color mode and user theme
  try { colorMode = localStorage.getItem(COLOR_MODE_KEY) || "auto"; } catch (e) {}
  try { userThemeId = localStorage.getItem(STORAGE_KEY + "-user") || saved; } catch (e) {}

  // Load all themes from server, then apply properly
  loadThemes().then(function () {
    // After themes are loaded, apply auto mode if needed
    if (colorMode !== "auto" || colorMode === "auto") {
      applyAutoMode();
    }
  });
